...
export default async function OverviewPage() {
  if (!isDatabaseConfigured()) {
    return (
      <EmptyState
        ...
        description="Provision a Postgres database (Neon via Vercel Marketplace) and set DATABASE_URL, then run `npm run db:push` to create the schema."
      />
    );
  }

  let data: {
    output: Awaited<ReturnType<typeof getMonthlyOutputSummary>>;
    capacity: Awaited<ReturnType<typeof getCapacityUtilization>>;
  ...
  try {
    ...
    data = { output, capacity, batchesBehind };
  } catch (e) {
    error = e;
  }

  if (error || !data) {
    return <EmptyState icon={Database} title="Couldn't load KPIs" description={describeDbError(error)} />;
  }
